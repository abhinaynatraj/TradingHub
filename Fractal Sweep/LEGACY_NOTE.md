# Fractal Sweep — Legacy Note

> **2026-04-19 consolidation:** This file originally lived in a separate
> `Fractal Sweep Legacy/` folder. On 2026-04-19 that folder was merged into
> `Fractal Sweep/` — the Fixed Constant and TTFM engines were removed, and
> the Legacy engine (this one) became the single canonical sweep+CISD
> implementation. The text below is preserved verbatim as the historical
> record of how this engine diverged from the pre-TTFM snapshot. Any
> instructions referring to a `Fractal Sweep Legacy/` folder or a DB symlink
> are no longer applicable — the DB lives directly in `Fractal Sweep/`.
