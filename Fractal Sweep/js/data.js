...
function makeDemoModel(key, label, mode, cisd, wrBase, evBase, pfBase, riskMed, spd) {
  ...
  const by_dow = [1, 2, 3, 4, 5].flatMap((dow, di) => {
    ...
    return ['LONG', 'SHORT'].map((dir, diri) => ({
      dow, dow_name: dn, direction: dir,
...
function getFilteredD(D) {
  ...
  function aggGroup(groups) {
    return Object.values(groups).map(g => {
      ...
      return {
        ...g, wr: +gwr.toFixed(3), ev: +gev.toFixed(3), pf: +gpf.toFixed(3),
  ...
  trades.forEach(t => {
    ...
    if (!comboMap[key]) {
      ...
      comboMap[key] = {
        hr: t.hr, dow: t.dow, dow_name: dn, direction: t.direction,
