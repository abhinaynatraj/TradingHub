...
Post-Phase-4: JSON no longer contains recent_trades; parquet is the only
source. Tests read from model_stats.parquet via the same logic the server uses.
"""
import json
import math
...
@pytest.mark.parametrize("model", MODELS)
@pytest.mark.parametrize("period", PERIODS)
def test_n_trades_matches(snapshot, model, period):
    if model not in snapshot:
        pytest.skip(f"model not in snapshot: {model}")
...
@pytest.mark.parametrize("model", MODELS)
@pytest.mark.parametrize("period", PERIODS)
def test_excursion_percentiles_match(snapshot, model, period):
    if model not in snapshot:
        pytest.skip(f"model not in snapshot: {model}")
    trades = _trades_from_parquet(model, period)
